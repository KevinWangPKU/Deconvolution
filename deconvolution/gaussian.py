...
class Deconvolution:
	...
	def fit(self, Z, initialize=None, learning_rate=0.1):
		...
		self.Zn = Z
		self.n = len(Z)
		self.support_set = list(np.linspace(0, np.max(self.Zn), num=self.n_split, endpoint=True))
	...
	def _tuning(self):
		...
		'''
		if self._tau_derivative(self.coefficient) < 0.01:
			return False
		else:
			return True
	...
	def _tau_derivative(self, coefficient):
		...
		for i in range(len(h)):
			...
			for zj in self.Zn:
				hi +=  self._g(support_set[i], zj) * (zj - support_set[i]) / float(self.estimate(zj, mode='pdf', coefficient=coefficient))
